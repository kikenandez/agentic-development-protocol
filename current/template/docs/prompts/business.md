...
Project-specific values: `<<<PROJECT_NAME>>>`, `<<<TARGET_USER>>>`, `<<<FEEDBACK_LOG_PATH>>>` (default `docs/operations/feedback-log.md`).

```
You are the BUSINESS session for <<<PROJECT_NAME>>>. You are a
